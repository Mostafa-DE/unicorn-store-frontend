  ...
  return (
    ...
    >
      <TabContext value={tabs}>
        <Box
          className={styles.containerTabsHeader}
        ...
        >
          ...
          >
            ...
            {user && <Tab value="3" label="Add Review" />}
          </Tabs>
        </Box>
        <TabPanel value="1">
          <Box display="flex" justifyContent="center">
            {product.description ? (
              ...
              <p className={styles.noDescriptionText}>
                Sorry, there is no description to show right now 🙂
              </p>
            )}
          </Box>
        </TabPanel>
        ...
        <TabPanel value="2">
          {reviews.length !== 0 ? (
            <Box className={styles.containerReviews}>
              {reviews?.map((review) => (
                <Box key={review.id} className={styles.containerCardReview}>
                  ...
                  <Box display="flex">
                    ...
                    >
                      ...
                      <span className={styles.created_at}>
                        Created: {reverseString(review.created_at)}
                      </span>
                      <span className={styles.update_at}>
                        Last Update: {reverseString(review.updated_at)}
                      </span>
                    </Box>
                  </Box>
                  <Box width="15rem" sx={{ margin: "0.6rem 0 0 0" }}>
                    ...
                    editReview.id === review.id ? (
                      ...
                      <span className={styles.bodyReviewText}>
                        {review.body}
                      </span>
                    )}
                  </Box>
            ...
            <p className={styles.noReviewsText}>
              There are no reviews to show for now 😞
            </p>
          )}
        </TabPanel>
        ...
        <TabPanel value="3">
          <Box display="flex" justifyContent="center">
            {!isUserAddReviewBefore(reviews) ? (
              ...
              <p className={styles.userAlreadyAddedReviewText}>
                Sorry you already added a review on this product 😉
              </p>
            )}
          </Box>
        </TabPanel>
